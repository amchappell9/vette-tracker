    ...
    <RadioGroup.Option
      ...
      className={({ active, checked }) =>
        `${className} flex h-full cursor-pointer flex-col rounded border border-gray-100 shadow outline-hidden transition duration-300 hover:-translate-y-0.5 hover:shadow-lg focus:-translate-y-0.5 focus:shadow-lg ${
          active ? "ring-opacity-80 ring-2 ring-red-500 ring-offset-2" : ""
        } ${checked ? "ring-opacity-80 ring-2 ring-red-500 ring-offset-2" : ""} `
      }
    >
      {({ checked }) => (
        <>
          ...
          <div className="flex-1 bg-gray-50 px-4 pt-2 pb-6">
            ...
            <ul className="mt-2 grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-x-4 gap-y-2">
